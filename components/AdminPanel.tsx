...
import { getUsers, saveUsers, getKnowledgeFiles, addKnowledgeFile, deleteKnowledgeFile } from '../services/dbService';
...
const AdminDashboard: React.FC = () => {
  ...
  const loadData = useCallback(() => {
    setUsers(getUsers());
    setKnowledgeFiles(getKnowledgeFiles());
  }, []);
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files) {
      ...
      loadData();
      // Clear statuses after a few seconds
  ...
  return (
    <div className="flex justify-center items-start min-h-screen w-full p-4 md:p-8 pt-24">
      <div className="w-full max-w-6xl p-6 md:p-8 space-y-8 bg-white/80 backdrop-blur-md rounded-2xl shadow-xl border border-gray-200">
